...
frontendControllers = {
	...
	results: function (req, res, next) {
        ...
        var paths = templates.getActiveThemePaths(req.app.get('activeTheme'));
        
        var data = {};
        for(var k in res.datatemplate){
            if(res.datatemplate.hasOwnProperty(k)){
                data[k] = res.datatemplate[k];
            }
        }
        if (res.Rerror){
            data.error = res.Rerror;
            data.errorcode = res.Rerrorcode;
            console.log(data.Rerror);
            return errors.renderErrorPage(data.errorcode,data.error,req,res,null);
        }
        if (res.error) {
            data.error = res.error;
            console.log(data.error);
            /*errors.renderErrorPage(data.errorcode,data.error,req,res,null);*/
            return next();
        } else {
            data.songlist = res.songlist;
            data.queuelist = res.queuelist;
        ...
        if(res.renderpath){
            if (paths.hasOwnProperty(res.renderpath+'.hbs')) {
                console.log("Path has property: "+res.renderpath+".hbs");
                return res.render(res.renderpath,data);
            }
        }
        
        if (paths.hasOwnProperty('results.hbs')) {
            console.log("Paths has property: results.hbs");
            return res.render('results', data);
        } else {
            console.log("Rendering Default Page: "+defaultPage);
            if(defaultPage.hasOwnProperty('results.hbs')){
